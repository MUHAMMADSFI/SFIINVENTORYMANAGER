package com.example.productbuilder.util;
...
public class HelperMethods {
    ...
    public static void writeMapToCSV(Map<String, Double> map, Map<String, String> desc, String fileName,
        ...
        try (FileWriter writer = new FileWriter(file)) {
            ...
            for (Map.Entry<String, Double> entry : map.entrySet()) {
                writer.write(entry.getKey() + "," + desc.getOrDefault(entry.getKey(), "description not available") + ","
                        + entry.getValue() + "\n");
            }
    ...
    public static void Shortage_Item_Wise(Map<String, Double> map, Map<String, Object[]> pipeline,
        ...
        try (FileWriter writer = new FileWriter(file)) {
            ...
            for (Map.Entry<String, Double> entry : map.entrySet()) {
                ...
                writer.write(entry.getKey() + "," + desc.getOrDefault(key, "description not available") + ","
                        + entry.getValue() + ","
    ...
    public static void createCsvOfUnbuiltItems(Set<BOM> unbuilt, Map<String, Double> inventory,
            ...
            String folderName) {
        String fileName = "Items_Shortage_List.csv";
        ...
        try (FileWriter writer = new FileWriter(file)) {
            ...
            for (BOM bom : unbuilt) {
                for (Map.Entry<String, Double> map : bom.getrequiredItems().entrySet()) {
                    ...
                    if (map.getValue() * (bom.getMinQuantity() - builtProducts.getOrDefault(bom.getProductId(), 0.0)) > inventory
                            .get(key)) {
                        writer.write(bom.getProductId() + ","
                                ...
                                + ","
                                + key + ","
                                + description.getOrDefault(key, "description not available").replaceAll(",",
                                        "")
                                + ","
                                + map.getValue() + ","
                                ...
                                + (map.getValue() * (bom.getMinQuantity() - builtProducts.getOrDefault(bom.getProductId(), 0.0)))
                                + ","
                                + ((map.getValue() * (bom.getMinQuantity() - builtProducts.getOrDefault(bom.getProductId(), 0.0)))
